...
import { ArrowLeft, X, CheckCircle2, AlertCircle, Loader2, Share2, Calendar, Clock, MessageSquare, Info, TrendingUp, Building2 } from "lucide-react"
import { rechargeInvestment } from "@/lib/actions/investments"

type Step = "amount" | "confirm" | "success" | "error"

export default function RechargeInvestmentScreen() {
  ...
  // Simulated wallet balance (should come from context/action in a real app)
  ...
  const handleTransfer = async () => {
    setLoading(true)
    try {
      const res = await rechargeInvestment(parseFloat(amount))
      if (res.success) {
          ...
          <div className="flex-1 flex flex-col bg-[#681984]">
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="flex items-end gap-1">
                ...
                <span className={`text-[56px] font-medium leading-none ${amount ? 'text-white' : 'text-white/30'}`}>
                  {amount || "0"}
                </span>
              </div>
            </div>
            
            <div className="text-center text-white/60 text-[13px] mb-6">
              Saldo disponible: S/ {walletBalance.toFixed(2)}
            </div>
